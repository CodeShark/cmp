...
void  cmp_int_add(cmp_int* r, cmp_int* a, cmp_int* b)
{
...
{
    unsigned int size_ = CT_MAX(a->size, b->size);
    r->size = 2*size_;
    r->sign = a->sign * b->sign;
    cmp_uint64_mul(r->limbs, a->limbs, b->limbs, size_);
}

void cmp_uint64_gcdext(uint64_t g[], uint64_t x[], int* signx, uint64_t y[], int* signy, uint64_t a[], uint64_t b[], unsigned int size)
{
    uint64_t nextx[MAX_LIMBS];
    uint64_t nexty[MAX_LIMBS];
    cmp_uint64_set_zero(nextx, size);
    cmp_uint64_set_zero(nexty, size);       nexty[0] = 1;
    int signnextx = 0;
    int signnexty = 1;

    cmp_uint64_set_zero(x, size);           x[0] = 1;
    cmp_uint64_set_zero(y, size);
    *signx = 1;
    *signy = 0;

    uint64_t q[MAX_LIMBS];
    uint64_t qprod[MAX_LIMBS];
    uint64_t b_[MAX_LIMBS];
    cmp_uint64_copy(g, a, size);
    cmp_uint64_copy(b_, b, size);

    while (!cmp_uint64_is_zero(b_)) {
        cmp_uint64_copy(a, b, size);
        cmp_uint64_tdiv_qr(q, b_, g, b_, size);
        cmp_uint64_mul(qprod, q, nextx, size);

        int signprevx = *signx;
        int signprevy = *signy;
        *signx = signnextx;
        *signy = signnexty;
        // both prevx and nextx have the same sign
        if (signprevx * signnextx > 0)
            sign = cmp(
    }
}
